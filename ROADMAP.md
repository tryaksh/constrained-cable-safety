...
| Can the code being tested read the answer key? | No, and the guard that stops it is itself tested: a deliberately cheating arm is run to confirm it fires. Zero events on real runs across v4, v5 and v6. [controls](evidence/cable_perception_controls_v4.json) |
...
| Can a study ask a question it is too coarse to answer? | No. The check runs before a contract can be frozen, and refuses rather than reporting a verdict it cannot resolve: v4 **refused 3 of 15** comparisons outright. v3, which predates the check, is the reason it exists. |
...
| Item | State |
| --- | --- |
| The rig | Drawn in FreeCAD **from the config file**, not by hand, which is why the picture at the top of [README.md](README.md) and the physics cannot disagree: 8 parts, all watertight, all one solid after the fuse, 3,704 facets. [record](evidence/cable_cell_cad_v6.json), meshes in `assets/cell_v6/`. |
...
| Item | State |
| --- | --- |
| **The page** | Generated by `scripts/build_showcase.py` from ten committed records, so it cannot drift from them — change a number in a record, rebuild, and the page changes. `artifacts/showcase/index.html` is generated output: it is not in the repository and nothing here serves it. It was published once as a **private** page and the record carries that link; it is not a public site and is not what this repository is for. The record of what was built is committed. [record](artifacts/showcase/page.json) |
...
- The working tree arrived broken: a deleted helper module that two live scripts
  still imported. No unit test noticed, because none of them import those
  scripts. *Where the documents had drifted* below says what now stops it.
...
- **The peg campaign's raw runs and its code are both elsewhere**, and
  [docs/PEG_INSERTION.md](docs/PEG_INSERTION.md) says where and what that costs.

## History

This repository was cut out of a branch of the orbital repository that no longer
exists, taking the peg-insertion campaign's records with it.
[docs/REPO_MAP.md](docs/REPO_MAP.md) records every branch, where it went and how
to reach the archived one; [docs/PEG_INSERTION.md](docs/PEG_INSERTION.md)
