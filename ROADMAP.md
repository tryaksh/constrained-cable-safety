...
**This is a record, not a plan.** Every question this project set out to answer
has been answered, and the work that turned those answers into something usable
is finished too. Nothing is queued.
...
| Item | State |
| --- | --- |
| **The page** | Generated by `scripts/build_showcase.py` from ten committed records, so it cannot drift from them — change a number in a record, rebuild, and the page changes. Published privately as an Artifact. [record](artifacts/showcase/page.json) |
...
Earlier closed work — a retired peg-insertion study, the first free-cable cycle,
and an earlier adaptive-sampling design — stayed behind in the space-robotics
repository this was cut from, with their evidence unchanged.
[evidence/INDEX.json](evidence/INDEX.json) lists every record kept here, each
with its own declared scope.
